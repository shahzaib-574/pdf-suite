...
export async function runDocxToPdfSelfCheck(): Promise<void> {
  const bad = await docxToPdf(
    pickedDocx(new Uint8Array([1, 2, 3, 4]), "nope.txt"),
  );
  ...
  if (missing.ok || !/docx/i.test(missing.message)) {
    throw new Error(
      `expected missing document.xml error, got ${JSON.stringify(missing)}`,
    );
  }
  ...
  if (
    !emptyDoc.ok ||
    emptyDoc.filename !== "sample.pdf" ||
    !emptyDoc.pageCount
  ) {
    throw new Error(
      `empty doc should still yield a PDF, got ${JSON.stringify({
        ok: emptyDoc.ok,
  ...
  const rich = await docxToPdf(
    pickedDocx(await buildDocx({ documentXml: richXml })),
  );
  ...
  if (!named.ok || named.filename !== "Invoice-2024.pdf") {
    throw new Error(
      `expected Invoice-2024.pdf, got ${named.ok ? named.filename : "err"}`,
    );
  }
  ...
  const tableFile = pickedDocx(
    await buildDocx({ documentXml: tableXml }),
    "grid.docx",
  );
  ...
  const trailingBlanks = await docxToPdf(
    pickedDocx(
      await buildDocx({
        documentXml: documentXml(
          paragraphXml("Only line") + "<w:p/>".repeat(80),
        ),
      }),
    ),
  );
  ...
  const cjk = await docxToPdf(
    pickedDocx(
      await buildDocx({ documentXml: documentXml(paragraphXml("你好世界")) }),
    ),
  );
  if (cjk.ok || !cjk.message.includes("cannot preserve")) {
    throw new Error("Unsupported characters must not produce a corrupted PDF.");
  }
  ...
  if (!unicode.ok || !unicode.pageCount)
    throw new Error(`Bundled Unicode fonts failed: ${JSON.stringify(unicode)}`);
}
