...
The screenshot fixture is generated by
`scripts/generate_screenshot_fixture.py` using the exact Python packages in
`scripts/requirements-screenshot-fixture.txt`. Its `--check` mode regenerates to
a temporary path and byte-compares the result, while `npm run pdf-selfcheck`
pins the reviewed bytes, page count, orientations, metadata, visible text, and
PDF worker compatibility. Install the generator requirements once, verify the
committed artifact, and regenerate only when intentionally changing it:
...
After regeneration, render and visually inspect all five pages before
committing the new binary.
